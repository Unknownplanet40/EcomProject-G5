...
<head>
    ...
    <title>Prepering Products</title>
        ...
        <div class="table-responsive">
            <table class="table table-striped-columns table-bordered table-hover d-none" id="ProductTable" style="width: 100%;">
                ...
                <tbody class="table-group-divider" id="ProductTableBody">
                    <?php
                    $stmt = $conn->prepare("SELECT * FROM user_orders WHERE Status = 'pending' ORDER BY Created_At DESC");
                    $stmt->execute();
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        while ($row_item = $result_details->fetch_assoc()) {
                            ...
                            if ($row_item['Quantity'] > $row_stock['Stock']) {
                                $QTYColor = 'table-danger';
                            } else {
                                $QTYColor = '';
                            }
                    ?>
                            <tr style="cursor: pointer;" class="table-row user-select-none <?php echo $QTYColor ?>">
